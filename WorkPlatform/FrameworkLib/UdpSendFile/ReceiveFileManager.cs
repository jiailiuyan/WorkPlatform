...
namespace UdpSendFiles
{
    ...
    public class ReceiveFileManager : IDisposable
    {
        ...
        public bool IsFloder { get; private set; }
        ...
        public ReceiveFileManager(bool isfloder, string md5, string path, string fileName, long partCount, int partSize, long length, IPEndPoint remoteIP)
        {
            ...
            get
            {
                if (_timer == null)
                {
                    _timer = new Timer(new TimerCallback(delegate(object obj)
                        ...
                        null, Timeout.Infinite, _interval);
                }
                return _timer;
            }
        }
        ...
        private void Create()
        {
            if (this.IsFloder)
            {
                _safeStream = new MemoryStream();
            }
            else
            {
                _tempFileName = string.Format("{0}\\{1}{2}", _path, _fileName, FileTemptName);
                var fileinfo = new FileInfo(_fileName);
                int count = 0;
                while (true)
                {
                    var _tempfileinfo = new FileInfo(_tempFileName);
                    try
                    {
                        if (!_tempfileinfo.Directory.Exists)
                        {
                            var samenameinfo = new FileInfo(_tempfileinfo.Directory.FullName);
                            if (samenameinfo.Exists)
                            {
                                samenameinfo.Delete();
                            }
        ...
        private void EndWrite(IAsyncResult result)
        {
            ...
            if (index == _partCount)
            {
                ...
                if (!this.IsFloder)
                {
                    ...
                    _success = _md5 == Jisons.MD5Helper.GetFastMD5(FileName);
                }
                else
                {
                    _success = _md5 == Jisons.MD5Helper.GetFastMD5(FileStream);
                }

                OnReceiveFileComplete(new FileReceiveCompleteEventArgs(_success));
            }
        }
        ...
        private void GetFileName()
        {
            ...
            {
                _fullName = string.Format("{0}\\{1}", _path, _fileName.Insert(index, nameIndex.ToString("_0")));
                nameIndex++;
            }
        }
