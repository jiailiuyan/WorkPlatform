...
namespace UdpSendFiles
{
    ...
    public class SendFileManager : IDisposable
    {
        ...
        public SendFileManager(Stream fs, string fileName)
        {
            _fileName = fileName;
            Create(fs);
        }
        ...
        {
            _fileStream = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.Read, _partSize * 10, true);
            Create(_fileStream);
        }

        private void Create(Stream _fileStream)
        {
            ...
            //MD5 验证影响速度
            _md5 = Jisons.MD5Helper.GetFastMD5(_fileStream);
        }
